...
export function Header() {
  ...
  useEffect(() => {
    ...
    const handleHashChange = () => {
      const hash = window.location.hash || (navItems.length > 0 ? navItems[0].href : '');
    ...
    // Call handlers once on mount to set initial active link
    handleScroll();
    handleHashChange();
  ...
  return (
    <header className="header">
      <nav className="container nav">
        <a href="index.html" className="nav__logo">Gelzieny.dev</a>
        ...
        <div className={isMenuOpen ? "nav__menu show-menu" : "nav__menu"}>
          ...
          <FaTimes className="nav__close" onClick={() => setIsMenuOpen (!isMenuOpen)} />
        </div>
